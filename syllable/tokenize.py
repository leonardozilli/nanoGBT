...
from common.tokenizer import SyllableTokenizer
...
def build_tokenizer(texts):
    """Build syllable tokenizer"""
    all_raw_syllables = []

    for text in texts:
        all_raw_syllables.extend(syllabify_text(text))

    unique_tokens = sorted(list(set(all_raw_syllables)))

    if "<UNK>" not in unique_tokens:
        unique_tokens.insert(0, "<UNK>")

    tokenizer = SyllableTokenizer()
    tokenizer.itos = {i: ch for i, ch in enumerate(unique_tokens)}
    tokenizer.stoi = {ch: i for i, ch in enumerate(unique_tokens)}
    tokenizer.vocab_size = len(unique_tokens)
    tokenizer.special_tokens["UNK"] = "<UNK>"
...
@click.option(
    "--out-dir",
    default="data/encoded/syllable/sonnets",
...
def main(data_dir, pretrain_data, out_dir):
    ...
    print(f"Vocabulary size: {vocab_size:,} syllables")
    ...
    vocab["special_tokens"].update(SPECIAL_TOKENS)
